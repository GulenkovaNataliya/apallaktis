...
## 0. ВАЖНЫЕ ПРАВИЛА РАЗРАБОТКИ

- **НЕ использовать localhost** — проект просматривается только через Vercel
- После коммита всегда делать `git push` для деплоя
...
- `/[locale]/dashboard/trash` — "Корзина пуста"

### Переводы (8 языков)

Тексты пустых состояний определены в `messages.ts`:
- `noCategories`, `noExpenses`, `noMethods`, `noObjects`, `noPayments`, `empty`

---

## 15.1 ⚖️ ЗАКОН ПОСТОЯННЫХ ОРАНЖЕВЫХ ПОДСКАЗОК

### Описание

Постоянные подсказки (hints), которые всегда видны и НЕ исчезают.

**Отличие от пустых состояний:**
- Пустые состояния (раздел 15) — исчезают когда появляются элементы
- Постоянные подсказки — видны всегда

### Правила

1. **Цвет текста**: `var(--orange)` (#ff6a1a)
2. **Элемент**: `<p>`, НЕ `<button>`
3. **Отступ от кнопки**: по закону кнопок (gap-12 = 48px)
4. **Шрифт**: `text-button` (16px, font-weight: 600)
5. **Выравнивание**: по центру (`text-center`)

### Код

```jsx
{/* Постоянная оранжевая подсказка */}
<p className="text-center text-button" style={{ color: 'var(--orange)' }}>
  {t.hintText}
</p>
```
...
- `/[locale]/objects` — "Удалённый объект доступен 30 дней"

---

## 15.2 ⚖️ ЗАКОН ДВУСТРОЧНЫХ ПОДСКАЗОК

### Описание

Подсказки из двух строк с уменьшенным межстрочным интервалом.

### Правила

1. **Контейнер**: `<div>` с двумя `<p>` внутри
2. **Цвет текста**: `var(--zanah)` или `var(--orange)` в зависимости от назначения
3. **Межстрочный интервал**: стандартный (line-height), БЕЗ дополнительного gap
4. **Выравнивание**: по центру (`text-center`)
5. **Отступ от других элементов**: gap-12 (48px) — как между кнопками

### Код

```jsx
{/* Двустрочная подсказка — межстрочный интервал МЕНЬШЕ чем между кнопками */}
...
| Элемент | Расстояние |
|---------|------------|
| Между кнопками | `48px` (gap-12) |
| Между строками внутри подсказки | `~20px` (line-height: 1.2) |

### Применяется на:

- `/[locale]/objects` — "Для финансового анализа" + "нажмите на объект"
- `/[locale]/dashboard/trash` — "Объект автоматически удаляется" + "через 30 дней"
...
**Проблема:** Голосовой ввод дублировал текст 5-6 раз на мобильных телефонах.

**Причина:** `event.resultIndex` может сбрасываться на мобильных устройствах при перезапусках recognition.

**Решение:** Добавлен `processedResultsRef` для независимого отслеживания обработанных результатов:
...
**Файлы изменены:**
- `global-expenses/page.tsx` — 1 функция голосового ввода
- `objects/[id]/finance/page.tsx` — 3 функции голосового ввода

---

### 2. Замена email-отчётов на кнопки скачивания

**Было:** Кнопка "Отправить отчёт на email" → модальное окно → отправка

**Стало:** Две кнопки "Скачать Excel" и "Скачать PDF"

**Дизайн кнопок:**

| Кнопка | Фон | Цвет текста | Иконка |
|--------|-----|-------------|--------|
| Скачать Excel | `var(--zanah)` | `var(--deep-teal)` | 📊 |
| Скачать PDF | `var(--orange)` | white | 📄 |

**Файлы изменены:**
- `analysis/page.tsx` — удалён email-модал, добавлены кнопки скачивания
- `objects/[id]/finance/page.tsx` — аналогично
- `lib/messages.ts` — переводы `downloadExcel`, `downloadPdf` для всех 8 языков

---

### 3. Анализ расходов по категориям на global-expenses

**Добавлено:** Группировка расходов по категориям с раскрывающимися блоками (как на странице объекта).

**Элементы:**

| Элемент | Фон | Цвет текста |
|---------|-----|-------------|
| Заголовок категории | `var(--polar)` | `var(--deep-teal)` |
| Сумма категории | — | `var(--deep-teal)` |
| Счётчик (Nx) | — | `var(--deep-teal)` opacity 0.7 |
| Стрелка ▲/▼ | — | `var(--deep-teal)` |
| Элементы расходов | rgba(0,0,0,0.05) | `var(--deep-teal)` |
| Итого Расходов | `var(--zanah)` | `var(--orange)` |

**Файл:** `global-expenses/page.tsx`

---

### 4. Секция анализа с экспортом на global-expenses

**Добавлено в конец страницы:**

1. **Заголовок** — "Анализ общих расходов" (`<p>`, НЕ кнопка)
2. **Выбор периода** — два input type="date" (От / До)
3. **Кнопки скачивания** — Excel и PDF

**Дизайн:**

| Элемент | Стиль |
|---------|-------|
| Заголовок | `text-xl font-bold`, цвет `var(--polar)` |
| Лейблы От/До | `text-sm`, цвет `var(--polar)` |
| Input date | border: `2px solid var(--polar)`, minHeight: 44px |
| Кнопка Excel | фон `var(--zanah)`, текст `var(--deep-teal)` |
| Кнопка PDF | фон `var(--orange)`, текст white |

**Экспорт включает:**
- Расходы за выбранный период
- Группировка по категориям с суммами
- Общая сумма расходов

**Переводы добавлены для всех 8 языков:**
- `analysisTitle`, `dateFrom`, `dateTo`, `downloadExcel`, `downloadPdf`, `totalExpenses`, `byCategory`

**Файлы изменены:**
- `global-expenses/page.tsx`
- `lib/messages.ts`
...
## 25. ⚖️ ЗАКОН СТРАНИЦЫ ИНСТРУКЦИИ (/[locale]/help)

Инструкция пользователя — единый формат для всех 8 языков.

### Структура файлов

```
frontend/public/help/
├── language-select.jpg          # Общий скриншот (для всех языков)
├── ru/
│   ├── Instruction.ru.md        # Русская инструкция
│   └── 1.ru.jpg ... 38.ru.jpg   # Скриншоты на русском
├── el/
│   ├── Instruction.el.md
│   └── 1.el.jpg ... 38.el.jpg
└── [другие языки: en, uk, sq, bg, ro, ar]/
```
...
|---------|--------|--------|
| Инструкция | `Instruction.{locale}.md` | `Instruction.ru.md` |
| Скриншоты | `{номер}.{locale}.jpg` | `28.ru.jpg` |
| Общие скриншоты | `{название}.jpg` (в корне help/) | `language-select.jpg` |

### Структура инструкции (сверху вниз)

```
1. <h1> Заголовок — оранжевый
2. <img> Логотип — по центру, 450px
3. <p> Вступительный текст — 3 строки, жирный, по центру
4. <p> ΑΠΑΛΛΑΚΤΗΣ (оранжевый) + перевод (белый)
5. <p> Слоган (оранжевый) + перевод (белый)
6. Предупреждение (blockquote)
7. --- разделитель
8. Разделы 1-9 с контентом и скриншотами
```
...
**Правила:**
- Три строки, разделённые `<br>`
- Без точки в конце
- Центрирование: `align="center"`
- Жирный: `<strong>`
...
**Правила:**
- Без тире между названием и переводом
- Без заглавных букв в переводе (кроме первой)
- Без восклицательного знака в переводе слогана
- Межстрочное расстояние: `line-height: 1.4`
...
| Заголовок h1 | `#F28C28` (оранжевый) |
| ΑΠΑΛΛΑΚΤΗΣ, слоган | `#F28C28` (оранжевый) |
| Переводы | `white` |
| Основной текст | `var(--polar)` |

### Скриншоты в тексте

```html
<img src="/help/language-select.jpg" alt="Описание" width="100%">
<img src="/help/ru/1.ru.jpg" alt="Описание" width="100%">
```

**Правила:**
- Путь начинается с `/help/`
- `width="100%"` — всегда
...
| # | Раздел | Подразделы |
|---|--------|------------|
| 1 | Начало работы | 1.1-1.6 (Язык, Установка, Главная, Регистрация, Вход, Сброс) |
| 2 | Главная панель | 2.1 |
| 3 | Способы оплаты | 3.1-3.2 |
| 4 | Общие расходы | 4.1-4.4 |
| 5 | Объекты и финансы | 5.1-5.7 |
| 6 | Финансовый анализ | 6.1-6.2 |
| 7 | Личный кабинет | 7.1-7.3 |
| 8 | Демо, аккаунт и подписка | 8.1-8.4 |
| 9 | Команда и поддержка | 9.1-9.4 |

### Применяется для всех 8 языков

el, ru, uk, sq, bg, ro, en, ar (RTL)
