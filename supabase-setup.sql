...
-- Create sequence for account numbers starting from 1010
CREATE SEQUENCE IF NOT EXISTS account_number_seq START WITH 1010;

-- 2. Включаем Row Level Security (RLS)
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;

-- 3. Политика: пользователи могут видеть только свой профиль
CREATE POLICY "Users can view own profile"
  ON public.profiles FOR SELECT
  USING (auth.uid() = id);

-- 4. Политика: пользователи могут обновлять только свой профиль
CREATE POLICY "Users can update own profile"
  ON public.profiles FOR UPDATE
  USING (auth.uid() = id);

-- 5. Политика: автоматическая вставка профиля при регистрации
CREATE POLICY "Users can insert own profile"
  ON public.profiles FOR INSERT
  WITH CHECK (auth.uid() = id);

-- 6. Функция: автоматическое создание профиля при регистрации
